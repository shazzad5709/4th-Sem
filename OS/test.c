...
//#include"AEStables.h"
using namespace std;
unsigned char key[32] = "this is my kung "; //input key
...
void addRoundKey(int r)
{
    for(int i=0;i<columns;i++)
    {
        for(int j=0;j<4;j++)
        {
            state[j][i] ^= roundKey[r*columns*4+i*columns*j];
        }
    }
}
...
void mixColumns()
{
    ...
    {
        t1 = state[0][i];
        t2 = state[0][i] ^ state[1][i] ^ state[2][i] ^ state[0][3];
...
void encrypt()
{
    ...
    addRoundKey(r); //round=0, for the initial round key
